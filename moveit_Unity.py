        ...
        while not is_shutdown():
            ...
            if mode == "TRIAL":
                ...
                # three blocks on table, pick up block 2 and place it
                    ...
                    tutorial.pickup_plan_cartesian(request)
                    tutorial.reset_robot(0)
                # two blocks on table, pick up block 2 and place it
                elif action in {"k4", "K4"}:
                    request = generate_request("2", "3")
                    tutorial.pickup_plan_cartesian(request)
                    tutorial.reset_robot(0)
                # One block on table, pick up block 2 but drop it
                elif action in {"G4", "g4", "J4", "j4"}:   
